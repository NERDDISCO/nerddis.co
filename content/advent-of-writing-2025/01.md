...
# run this function in a worker on Runpod
@remote(
...
Earlier I said that `@remote` only changes _where_ your function runs when "something" calls it.

In this hello-world, that "something" is a tiny HTTP API based on FastAPI to expose `/gpu/hello`.

More generally, `say_hello` is just a normal async Python function.

Anything in your local Python app that can `await say_hello(...)` can kick off work on a GPU worker – for example another Python web framework, a CLI command, or a background job in a service.

### `/gpu`: router prefix

In `main.py`, the GPU router is mounted like this: `app.include_router(gpu_router, prefix="/gpu", ...)`.

This puts everything from `gpu_router` under `/gpu`.

The folder being called `gpu` is just a convention here – the route comes from this `prefix`, not from the directory name.

### `/gpu/hello`: FastAPI route that calls `say_hello`

Now update `workers/gpu/__init__.py`:

The `from .endpoint import say_hello` line imports the `say_hello` function from `workers/gpu/endpoint.py` – `.endpoint` is just the `endpoint.py` module next to this `__init__.py`.
...
When you send a POST request to `/gpu/hello`, FastAPI:

- parses the JSON body into `HelloRequest`
- calls `say_hello(request.name)`
- returns whatever `say_hello` returns

Combined with the `/gpu` prefix from `main.py`, this gives you the final path: `/gpu/hello`.

If you change the `prefix` or the `@gpu_router.post(...)` path, the URL will change accordingly.
...
When you send a request to `/gpu/hello` (via Swagger UI or curl), FastAPI calls the `hello` route, which calls `say_hello()` in `endpoint.py`.

Because `say_hello` is decorated with `@remote`, flash does the following instead of running it locally:
